...
- [ ] Teste feature: visitante GET `/` → 200, vê Entrar e ADPF
- [ ] Teste: autenticado GET `/` → vê atalho Mapa
- [ ] `HomeController`, rota pública `home`, remover redirect antigo
- [ ] `AuthenticatedSessionController` + `RedirectIfAuthenticated` → `home`
- [ ] Atualizar `AuthenticationTest` e `ExampleTest`

### Task 2: View + CSS

- [ ] `resources/views/home/index.blade.php` + classes `.home-*`
- [ ] Grade 3×2 autenticada; Entrar para guest
- [ ] Sidebar logo → `route('home')`
- [ ] `pint` + testes verdes
